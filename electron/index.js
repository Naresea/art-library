...
// how to avoid manual update of this?
const JAR = isDev
    ? 'art-library-backend/spring-server/target/art-library-backend-1.0-SNAPSHOT.jar'
    : 'extras/server/art-library-backend-1.0-SNAPSHOT.jar';
const JAVA_CMD = isDev
    ? './external/jdk-16.0.1/bin/java'
    : './extras/jdk/bin/java';
...
function startServer(port) {
  ...
  serverProcess = require('child_process')
    .spawn(JAVA_CMD, [ '-jar', server, `--server.port=${port}`, '--spring.profiles.active=electron']);

  serverProcess.stdout.on('data', logger.server);
  serverProcess.stderr.on('data', logger.serverError);
...
function stopServer() {
  ...
  axios.post(`${baseUrl}/api/actuator/shutdown`, null, {
...
function loadHomePage() {
  ...
  setTimeout(function cycle() {
    axios.get(`${baseUrl}/api/actuator/health`)
      .then(() => mainWindow.loadURL(`${baseUrl}`))
      .catch(e => {
